    ...
    //
     ...
     if(TAGR){
			...
			if(HEVT) {
                        ...
                        // double kp1P_Vz = kp1x.z();
                        // double kp1P_Ptot = kp1.V().r();
                        // double kp1P_Phi = kp1.V().phi();
                        // double kp1P_Theta = kp1.V().theta();
                        // char* Units = (char*)"radian";
                        // char* Effdir = (char*)"g12_corrections/EFFICIENCY_HISTS/";
                        // nt_value[70] = clas::g12::g12_proton_efficiency(kp1P_Vz, kp1P_Ptot, kp1P_Phi, kp1P_Theta, Units, Effdir);
                        // double pip1P_Vz = pip1x.z();
                        // double pip1P_Ptot = pip1.V().r();
                        // double pip1P_Phi = pip1.V().phi();
                        // double pip1P_Theta = pip1.V().theta();
                        // nt_value[71] = clas::g12::g12_pip_efficiency(pip1P_Vz, pip1P_Ptot, pip1P_Phi, pip1P_Theta, Units, Effdir);
                        // double pim1P_Vz = pim1x.z();
                        // double pim1P_Ptot = pim1.V().r();
                        // double pim1P_Phi = pim1.V().phi();
                        // double pim1P_Theta = pim1.V().theta();
                        // nt_value[72] = clas::g12::g12_pim_efficiency(pim1P_Vz, pim1P_Ptot, pim1P_Phi, pim1P_Theta, Units, Effdir);

                        // nt_value[73] = -1000;
                        // if(evt.N(PiPlus) == 2)
                        // {
                        //     double pip2P_Vz = pip2x.z();
                        //     double pip2P_Ptot = pip2.V().r();
                        //     double pip2P_Phi = pip2.V().phi();
                        //     double pip2P_Theta = pip2.V().theta();
                        //     nt_value[73] = clas::g12::g12_pip_efficiency(pip2P_Vz, pip2P_Ptot, pip2P_Phi, pip2P_Theta, Units, Effdir);
                        // }
                        // nt_value[74] = -1000;
                        // if(evt.N(PiMinus) == 2)
                        // {
                        //     double pim2P_Vz = pim2x.z();
                        //     double pim2P_Ptot = pim2.V().r();
                        //     double pim2P_Phi = pim2.V().phi();
                        //     double pim2P_Theta = pim2.V().theta();
                        //     nt_value[74] = clas::g12::g12_pim_efficiency(pim2P_Vz, pim2P_Ptot, pim2P_Phi, pim2P_Theta, Units, Effdir);
                        // }
